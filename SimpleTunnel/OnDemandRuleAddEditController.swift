	...
	override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
		...
		switch identifier {
			...
			case "edit-interface-type-match":
				...
				enumController.setValues(stringArray, title: "Interface Type", currentSelection: currentSelection) { newRow in
			...
			case "edit-on-demand-action":
				...
				let	stringValues = enumValues.flatMap { $0.description }
                let currentSelection = enumValues.firstIndex { $0 == targetRule.action }
                var stringArray: Array<String> = Array()
                for str in stringValues {
                    stringArray.append(String(str))
                }
				enumController.setValues(stringArray, title: "Action", currentSelection: currentSelection) { newRow in
